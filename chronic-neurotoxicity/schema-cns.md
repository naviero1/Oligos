...
re-mapping: `data/oligos.csv` uses the **identical 17-column layout** as
`data/oligos.csv`, and `data/measurements.csv` extends the measurement layout
...
| `source_id` | string | → entry in `SOURCES-CNS.md`. |
...
EDA/model training needs no join. `data/oligos.csv` and
`data/measurements.csv` remain the source of truth; regenerate this file
