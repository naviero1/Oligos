...
## Two findings the dataset is built around

**1. A near-controlled matched pair: volanesorsen vs olezarsen.**
...
**2. Mechanism must be labelled, not assumed.** `imetelstat` (Rytelo) causes
...
*Generated by `scripts/report_thrombo.py`. Regenerated after every ingestion
round so the documented numbers cannot drift from the data.*
<!-- END RECORD COUNTER -->
