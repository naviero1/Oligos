...
brief (`../_shared/sources/OligoTox_challenge_brief.pdf`, p. 1), alongside
...
This is one of **four curated datasets** in this repository, alongside
**[kidney toxicity](../kidney-toxicity/README.md)**,
**[chronic neurotoxicity](../chronic-neurotoxicity/README.md)** and
**[hydrocephalus](../hydrocephalus/README.md)**; see [`../README.md`](../README.md) for the
register. It reuses the kidney dataset's two-table design, controlled vocabularies,
provenance discipline and no-fabrication policy, so the datasets **join on oligo identity**.
Their *grades* do not: each endpoint's 0–3 scale is a separately written rubric, and the
columns must not be unioned or compared across datasets.
...
| — of which carry at least one measurement | **223** (29 oligo rows are referenced by no measurement) |
...
| Distinct target genes | **60** (62 raw `target_gene` values, less the `TBD` and `NA` placeholders) |
...
which covers every dataset in this repository.
