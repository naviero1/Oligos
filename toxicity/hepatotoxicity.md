...
histopathology. It is the first endpoint named in the Challenge brief's list, quoted verbatim in
[`README.md`](./README.md). **This project extracted no hepatotoxicity data.** Five PDFs are held under
`sources/hepatotox/`; none has produced a row. This file records what they contain, what they would yield, and why none of it was mined.
...
| Extraction status | no rows in `data/`; source tables parsed and cross-referenced (§3, §4); no grading rubric written for this endpoint |
| Deck coverage | no source, table or row from this endpoint appears; grep of `PRESENTATION.md` for `hepatotox\|Dieckmann\|Burdick\|Hagedorn` returns zero hits. The deck does argue the liver/kidney framing on two rendered slides — §5 item 8 |
...
Paths verified by directory listing of `sources/hepatotox/`; page counts read from the files.
...
| Locus | Content |
|---|---|
| `sources/kidney/US11105794_in_vitro_nephrotox_assay_patent.pdf` p.29, Table 5 | "EGF levels in culture media from primary hepatocytes at day 3 after oligonucleotide treatment", under Example 5. Rows are the patent's own compound numbers (saline, 1-1, 2-1, 3-1, 4-1, 10-1, 11-1, 14-1) with in-vivo grade words. |
| `sources/kidney/Moisan2017_EGF_uptake_nephrotox_ASO_invitro_PMC5363415.pdf` Fig. 3E (legend p.6, methods p.14) | "Measurement of EGF in the supernatant of human hepatocytes treated with 10 and 100 mM of AONs for 3 days"; the methods head the section "Human Hepatocytes and Retinal Pigment Epithelial Cells" and open "Cryopreserved human hepatocytes were suspended in William's Medium E". |

Multi-endpoint material also serving this endpoint (Frazier 2015; MMB 2434 Ch.27, mouse hepatocyte / Kupffer / LSEC isolation) is indexed in [`cross-cutting.md`](./cross-cutting.md).
...
`data/oligos.csv`, so they would be 11 new oligos. Dieckmann mmc2 Table 1 gives 6 rows with sequence, target, ALT
...
appears twice) plus Table S2, genes up-regulated by LNA41 — the same two tables as mmc1. `SOURCES.md:225` states the raw per-oligo table is a
...
2013, *Nucleic Acid Ther.* 23, 302–310 — the PDF already in `sources/hepatotox/`, whose own text reads "We
...
Table 1 must carry Hagedorn's DOI in `source_ref` (`sources/SOURCES.md:228-230`).

## 4. Data

No column in `data/measurements.csv` records a liver readout, and a scan of all 111 rows for `hepat`, `liver`, `_alt`
and `aspartate` returns exactly two hits, neither a measurement: `MSR010.source_ref = LiverInt2022_10.1111_liv.15090`
(a journal name) and `MSR044.notes = LNP_delivery_negative_control;grade_provisional`. `is_kidney_specific = TRUE` on
111 of 111 rows corroborates this but is not evidence — a mislabelled row would be invisible to a constant flag.
...
| Within US11105794 | Table 5, hepatocyte EGF (p.29) | Table 1 → all 21 `N3` rows |
...
field (73) "Assignee : Hoffmann - La Roche Inc."; `sources/SOURCES.md:181` for Dieckmann.
...
source data held outside `data/` — no script validates it, `build_merged.py` does not see it, the `schema.md` QC log
does not cover it, and ingestion rather than this dossier is where it would become a record. That is a departure from
the register's rule at [`README.md`](./README.md) line 9, recorded here rather than left implicit.
...
1. `SOURCES.md:76-78` frames the Dieckmann panel as "the single biggest *volume* source available → use for the
...
2. `README.md` § "Record counter" (the hepatotox-fallback row) ("not needed"), `METHODOLOGY.md:60-62` ("None ingested as rows yet") and `SOURCES.md:216-218`
   ("pending") together read as an absence of available data, when 17 per-oligo records are in hand. Exclusion is
   legitimate; it is simply not recorded as a decision at any of those three loci.
3. Of the five fallback candidates at `SOURCES.md:83-91`, Kasuya 2016 has a confirmed DOI (`10.1038/srep30377`) and is flagged open access at `:174-175` but was never acquired; only Creyon 2025 (`:89`) remains uncited.
4. The `is_kidney_specific = FALSE` branch reserved at `schema.md:61` is used by zero of 111 rows, so deliberate exclusion is indistinguishable from non-attainment.
5. `SOURCES.md:79` leaves N1's redistribution as "journal OA — `derived_features_only`; confirm". It was never confirmed, and `derived_features_only` and `verify` are used by zero rows, so neither has a worked precedent.
6. `nephrotox_grade` cannot be reused: `schema.md:74-77` is renal at every level — 0 "No renal signal at tested
   exposure", 1 low-MW proteinuria, 2 KIM-1/NGAL/clusterin or histopathology, 3 "acute kidney injury … renal failure / dialysis".
7. `METHODOLOGY.md:184-187`'s "case encodes chemistry" rule describes how sequences are stored in this repo's rows and
...
8. `PRESENTATION.md:167` and `:647` argue on rendered slides that open oligo-tox data is liver-focused and kidney under-served, without mentioning that five liver PDFs are held.
...
| The two in-repo hepatocyte panels | Never noticed; both PDFs are filed under `sources/kidney/` and were mined for kidney readouts only |
