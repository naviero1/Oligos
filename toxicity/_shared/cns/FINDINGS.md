...
## F-13 — Two of nine sources were recorded under the wrong licence, in opposite directions

Compiling the source register meant re-resolving every identifier against Europe PMC and reading
each article's own machine-readable `<permissions>` block instead of trusting the reading notes.
Two of the nine were wrong, and the errors ran in opposite directions — which is why neither was
going to surface on its own.

| | recorded | the article's own permissions block says |
|---|---|---|
| **K1** Miller 2024 | CC BY-NC → `cc_by_nc` | **CC BY-NC-ND 4.0**, on *both* the journal record (PMC11567125) and the preprint record (PMC11185713) |
| **L1** Kuroda 2025 | CC BY-NC → `cc_by_nc` | **CC BY 4.0** |

K1 was the damaging one: NoDerivatives means a restructured derivative cannot be licensed on, so
its 41 measurements were being offered under terms the authors never granted. They are now
`summary_stat_only`, the same class as HV3. L1 ran the other way — 6 measurements needlessly
restricted, now `cc_by`.

K1's citation was also wrong in two further ways: the title recorded ("Chemical and physical
variables influencing the acute tolerability…") is not a title either record carries, and the
journal DOI had been paired with the *preprint's* PMCID. The real title is "Preventing acute
neurotoxicity of CNS therapeutic oligonucleotides with the addition of Ca2+ and Mg2+ in the
formulation"; both records are now stated, and the registry says which one the supplementary
file was actually read from.

> `curl 'https://www.ebi.ac.uk/europepmc/webservices/rest/PMC11567125/fullTextXML' | grep -A3 permissions`
> `python3 qc/validate_dataset.py --json` → `redistribution_distribution`

`LICENSE.md` was quoting row counts from a release three revisions old (2,065 measurements
against the current 4,428) because it was hand-written. It is now generated by
`src/make_summary.py` from the released rows and the source registry, so it cannot drift again.

---

## F-14 — The endpoint split was silently dropping every oligonucleotide that carried no measurement

`write_split()` allocated oligos by the measurements that reach them. A compound with no
measurement therefore reached no endpoint folder and vanished from the release. **21 oligos were
being lost this way, 18 of them sequence-resolved, and all 21 human** — the exact class of row
this module is trying to publish.

They are not junk. A source can name a compound, print its sequence and per-position chemistry,
and then report its result only inside a pooled figure panel with no per-compound value. Reading
a number off that panel would be estimating it, so the measurement is genuinely absent while the
chemistry is genuinely published.

The defect hid behind a QC check that was passing for the wrong reason: *"every oligo has at
least one measurement"* passed because the split had already removed every counter-example
before the checker saw the data.

Both halves are fixed. A measurement-less oligo is now filed with the endpoint its own source
predominantly serves; `assemble.py` asserts that the split loses no oligo and no modification
record; and the QC check now reads *"every oligo either has a measurement or declares why it has
none"* — the declaration being a `CHARACTERISED_ONLY:` note written by the builder, so an
**accidental** orphan still fails while a **deliberate** one is explained in the row itself.

> `python3 src/assemble.py` → `1879 oligos and 32569 modification records all retained`
> `python3 qc/validate_dataset.py` → `every oligo either has a measurement or declares why it has none — 21 declared characterised-only`

Net effect on the release: **1,858 → 1,879 oligonucleotides**, sequences present **1,840 → 1,858**,
and 18 more human sequenced compounds in the *German's analysis* sheet.
