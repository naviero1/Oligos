...
index](./README.md#scope-authority). It is the only endpoint here that carries extracted data: `is_kidney_specific` is `TRUE` on 165 of 165 rows
of [`data/measurements.csv`](data/measurements.csv) and no `FALSE` row exists, so all 65 oligos and all 165 rows belong here and no measurement
...
| Measurement rows | 165 | `data/measurements.csv` (165 rows × 24 columns) |
...
| In-vitro volume | human in-vitro now the largest class (67 of 165, 40.6%) after the Table 2 extraction | `schema.md` QC log 2026-09-03 |
...
| Merged analysis view | [`data/oligotox_kidney_merged.csv`](data/oligotox_kidney_merged.csv) | 165 × 40; generated, not canonical (`schema.md:131-146`) |
