...
| Graded rows (0/1/2/3) | 942 — 382 / 462 / 72 / 26 |
| Structural QC | 36 / 36 checks pass |
| Numeric values located in their cited source | 1,862 / 1,862 |
...
| `unintended_toxicity` only — coagulation disturbance presented as an adverse effect | 300 |
| **both** — an on-target compound whose effect the source reports as harm | 144 |
| neither — context rows (assay controls, comparators, background) | 368 |
...
TT, ACT prolongation; fibrinogen decrease). The other 1,446 rows are **left ungraded**,
each stating why in `grade_basis`, rather than graded by an invented threshold. Every grade
is `provisional` pending subject-matter review, and every grade is reproducible from
`ratio_to_control` — a QC check re-derives all 942 and fails the build on any disagreement.
...
- **14 rows cite a supplementary PDF not held locally** and are therefore outside the
